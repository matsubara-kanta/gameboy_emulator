...
public class CPU implements Serializable {
    ...
    static class CB extends Operation {
        ...
        public int execute(CPU cpu) {
            int cbOpcode = cpu.mem.slowReadByte(cpu.regs.PC.read() + 1); 
    ...
    Readable SPr8() {
        byte r8 = (byte)mem.slowReadByte(regs.PC.read()+1); 
    ...
    int SUB(Readable toSubtract){
        ...
        regs.flags.setFlag(ZFLAG, (result == 0));
        regs.flags.setFlag(CFLAG, (diff < 0)); 
        regs.flags.setFlag(HFLAG, ((op1 & 0xf) - (op2 & 0xf) < 0)); 
        regs.A.write(result);
    ...
    int SBC(Readable toSubtract){
        ...
        regs.flags.setFlag(ZFLAG, (result == 0));
        regs.flags.setFlag(CFLAG, (diff < 0)); 
        regs.flags.setFlag(HFLAG, ((op1 & 0xf) - (op2 & 0xf) - carry < 0)); 
    ...
    int DEC(ReadWritable toDec){
        ...
        regs.flags.setFlag(ZFLAG, (result == 0));
        regs.flags.setFlag(HFLAG, (original & halfMask) < 1); 
    ...
    int JR(Readable offset){
        int location = regs.PC.read() + (byte)offset.read(); 
